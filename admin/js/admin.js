...
(function () {
  ...
  function fetchView(view, callback) {
    ...
    var url = API_URL + '?view=' + view + '&from=' + range.from + '&to=' + range.to;
    fetch(url, {
  ...
  function renderPages() {
    fetchView('pages', function (d) {
      var rows = (d.pages || []).map(function (p) {
        return [p.pathname, p.views, p.uniques, fmtTime(p.avgTimeMs)];
      });
  ...
  gateForm.addEventListener('submit', function (e) {
    ...
    sha256(pw).then(function (hash) {
      // Test against API
      var range = getDateRange();
      return fetch(API_URL + '?view=overview&from=' + range.from + '&to=' + range.to, {
